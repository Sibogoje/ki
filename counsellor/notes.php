        ...
        <div class="col-md-12 mb-4" style="padding: 20px;">
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    // Fetch counselors for sharing notes  
$counselors_query = "SELECT user_id, name, surname FROM users WHERE user_role = 'client'"; 
$counselors_result = $db->query($counselors_query);
                    ...
                    echo "<form action='share_note.php' method='post'>";
                    echo "<input type='hidden' name='note_id' value='" . $row['note_id'] . "'>";
                    echo "<select name='shared_with' class='form-select mb-3'>";
                    while ($counselor = $counselors_result->fetch_assoc()) {
                        echo "<option value='" . $counselor['user_id'] . "'>" . $counselor['name'] ."    ".$counselor['surname']. "</option>";
                    }
                    echo "</select>";
                    echo "<button type='submit' class='btn btn-secondary'>Share with Client</button>";
                    echo "</form>";
                    echo "</div></div>";
                }
            } else {
